...
$slider4 = '<div class="logo-style post_'.$post_id.'">
	<div class="logo-slider-style">
		<div class="slider-style slider-style-2 grid-style-1 blur-style owl-carousel owl-theme '.$slider_dots_nav.' '.$slider_nav_arrow.'" data-carousel-options="'.esc_attr( $owl_data ).'">';			
			if ( $que->have_posts() ) {
				while ( $que->have_posts() ) : $que->the_post();															
				    ...
				    $slider4 .= '<div class="logo-item-wrap">
			            <div class="logo-img">';

			               if ( has_post_thumbnail() ) {
								$slider4 .= '<a href="'.esc_url($rstlogo_link).'" style="padding:'.wp_kses_post($logo_padding).'"><img src="' . $logo_image . '" alt="' .get_the_title(). '"></a>';
							}

			               $slider4 .= '<div class="normal-text">
			                    '.$show_title.'';
